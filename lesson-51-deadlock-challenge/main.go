...
func fanOut(in <-chan int, n int) []<-chan int {
	// make a slice of chan int with len and capacity n (deadlock error will occur)
	// because make is used, an underlying array is allocated, initialized with zero-values of nil (zero value of chan int is nil)
	//xc := make([]<-chan int, n)
	var xc []<-chan int
	for i := 0; i < n; i++ {
